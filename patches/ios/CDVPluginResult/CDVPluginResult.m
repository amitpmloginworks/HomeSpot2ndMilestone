...
// patch a NS bug in cordova library
id messageFromDictionary(NSDictionary* theMessages)
{
    NSMutableDictionary* messages = [@{} mutableCopy];
    [theMessages enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        messages[key] = massageMessage(obj);
    }];
    return messages;
}
...
+ (CDVPluginResult*)resultWithStatus:(CDVCommandStatus)statusOrdinal messageAsDictionary:(NSDictionary*)theMessage
{
    // patch a NS bug in cordova library
    //return [[self alloc] initWithStatus:statusOrdinal message:theMessage];
    return [[self alloc] initWithStatus:statusOrdinal message:messageFromDictionary(theMessage)];
}
